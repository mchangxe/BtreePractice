 ...
 *                                         */
	...
	//parent's children are now new_left + new_right
	parent->children.insert(child_itr, new_right);
	
	//delete extra elements in new_left, since it is the original children vector
	new_left->elements.erase(mid_elem_itr, child->elements.end()); 

	//copy in children if exist
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
	...
	if (first_larger_idx!=subroot->elements.size() && subroot->elements[first_larger_idx].key == pair.key) return;
